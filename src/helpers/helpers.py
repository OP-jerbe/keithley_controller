...
def convert_usteps_to_microns(usteps: int, usteps_per_step: int = 64) -> float:
    if not isinstance(usteps, int):
        raise TypeError(f'Expected int for usteps arg but got {type(usteps).__name__}.')
    if not isinstance(usteps_per_step, int):
        raise TypeError(
            f'Expected int for usteps arg but got {type(usteps_per_step).__name__}.'
        )
    threads_per_inch = 40
    microns_per_rotation = threads_per_inch**-1 * 25400  # 635
    degrees_per_step = 1.8
    steps_per_rotation = 360 / degrees_per_step  # 200
    usteps_per_rotation = steps_per_rotation * usteps_per_step  # 200 x 64 = 12800
    microns = microns_per_rotation / usteps_per_rotation * usteps
    return round(microns, 2)


def convert_microns_to_usteps(microns: float, usteps_per_step: int = 64) -> int:
    if not isinstance(microns, (float, int)):
        raise TypeError(
            f'Expected float or int for usteps arg but got {type(microns).__name__}.'
        )
    if not isinstance(usteps_per_step, int):
        raise TypeError(
            f'Expected int for usteps arg but got {type(usteps_per_step).__name__}.'
    ...
    steps_per_rotation = 360 / degrees_per_step  # 200
    usteps_per_rotation = steps_per_rotation * usteps_per_step  # 200 x 64 = 12800
    usteps = usteps_per_rotation / microns_per_rotation * microns
    return int(round(usteps, 0))
...
def get_app_version() -> str:
    ...
    Attempts to fetch the version metadata for the 'stage_controller' package.
    ...
    """
    try:
        return version('stage_controller')
...
if __name__ == '__main__':
    ...
    port = config_data.get('COMPORT', 'port')
    print(port)
